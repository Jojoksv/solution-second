...
//   • Bin cleaning tasks: pending → assigné → en_cours → terminé
//   • Monotonic fill tracking: fill levels only ever increase until reset
//
// Uses module-level reactive pattern (no Zustand needed).
...
// Tracks the last known fill for each bin (monotonic enforcement)
const _prevFills: Record<string, number> = {}
// Bins currently being cleaned — their fill is suppressed to 0
...
// Call this for every fill value received from the API.
// Returns the stable (monotonically increasing) fill value.
...
// ── Task generation ────────────────────────────────────────────────────────
// Called every time new green data arrives. Generates tasks for bins >= 75%.
  ...
  sites.forEach(site => {
    site.zones.forEach(zone => {
      ...
      const alreadyActive = _tasks.some(
        t => t.bin_id === binId && (t.status === 'pending' || t.status === 'assigned' || t.status === 'en_cours')
      )
...
export function assignTask(taskId: string) {
  _tasks = _tasks.map(t =>
    t.id === taskId ? { ...t, status: 'assigned' as TaskStatus, assigned_at: Date.now() } : t
  )
  notify()
}
...
export function completeTask(taskId: string) {
  ...
  _tasks = _tasks.map(t =>
    t.id === taskId ? { ...t, status: 'terminé' as TaskStatus, completed_at: Date.now() } : t
  )

  // Mark bin for reset — fill immediately drops to 0
  const { bin_id } = task
  _resetBins.add(bin_id)
  _prevFills[bin_id] = 0
  notify()

  // After 5 s the bin is back in service (starts refilling)
  setTimeout(() => {
