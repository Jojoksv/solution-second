    ...
    <div className="panel p-4 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Icon
            size={14}
            className={isAlert ? 'text-[#E5484D]' : 'text-[#888888]'}
          />
          <span className="text-[12px] font-medium text-[#888888] tracking-wide uppercase">
            {title}
          </span>
        </div>
      </div>
      <div className="flex items-end justify-between mt-1">
        <span
          className={`text-2xl font-semibold tracking-tight ${isAlert ? 'text-[#E5484D]' : 'text-[#EDEDED]'}`}
        >
          {value}
        </span>
        {trend && (
          <div className="flex items-center gap-1 bg-[#141414] px-1.5 py-0.5 rounded-[4px] border border-[#2A2A2A]">
            <span
              className={`text-[11px] font-medium ${trend.startsWith('+') ? 'text-[#10B981]' : trend.startsWith('-') ? 'text-[#E5484D]' : 'text-[#888888]'}`}
            >
              {trend}
            </span>
          </div>
    ...
    <div className="panel flex flex-col">
      <div className="px-4 py-3 border-b border-[#2A2A2A] flex items-center justify-between">
        <h2 className="text-[13px] font-semibold text-[#EDEDED]">
          Sites en surveillance
        </h2>
        <span className="text-[10px] text-[#888888] font-mono">
          {density?.sites.length ?? 0} sites
        </span>
      </div>
          ...
          <tbody>
            {density?.sites.map((site) => {
              const statusColor =
                site.status === 'red'
                  ...
                  : site.status === 'orange'
                    ...
                    : '#10B981'
              const TrendIcon =
                site.rise_rate_10min_percent > 5
                  ? TrendingUp
                  : site.rise_rate_10min_percent < -5
                    ? TrendingDown
                    : Minus
              return (
                ...
                >
                  <td className="px-4 py-2.5 font-medium text-[#EDEDED] whitespace-nowrap">
                    {site.site_name}
                  </td>
                  <td className="px-2 py-2.5">
                    <span
                      style={{ color: cityColor(site.city) }}
                    ...
                    <div className="flex items-center justify-end gap-2">
                      <div className="w-12 bg-[#1A1A1A] rounded-full h-1 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${site.occupancy_percentage}%`,
                  ...
                  <td className="px-4 py-2.5 text-right">
                    ...
                    >
                      <TrendIcon size={11} />
                      <span>
                        {site.rise_rate_10min_percent > 0 ? '+' : ''}
                        {site.rise_rate_10min_percent}%
                      </span>
                    </div>
                  </td>
                </tr>
    ...
    <div className="flex flex-col gap-6 w-full max-w-[1400px] mx-auto">
      ...
      <div>
        ...
        <p className="text-[13px] text-[#888888] mt-1">
          Métriques temps-réel des capteurs déployés sur tous les sites JOJ
          Dakar 2026.
        </p>
      </div>
    ...
    <div className="flex flex-col">
      {logs.map((log) => {
        ...
        return (
          ...
          >
            <div
              className={`mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0 ${log.status === 'warn' ? 'bg-[#F5A623]' : 'bg-[#10B981]'}`}
            />
            <div className="flex flex-col w-full">
              <div className="flex items-center justify-between">
                <span className="text-[12px] text-[#EDEDED] leading-snug">
                  {log.msg}
                </span>
